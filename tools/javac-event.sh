...
# 見ているのは run-shifu/versions/26.2/paper-26.2.jar(前回の once.sh + run-server.sh の
...
if [ ! -f "$CP" ] || [ "$RUN/versions/26.2/paper-26.2.jar" -nt "$CP" ]; then
    python - "$CP" "$(cygpath -m "$RUN")" <<'PYEOF'
import glob, io, os, sys
run = sys.argv[2]
jars = [run + "/versions/26.2/paper-26.2.jar"] + glob.glob(run + "/libraries/**/*.jar", recursive=True)
