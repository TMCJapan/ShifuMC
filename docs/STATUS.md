...
|---|---|---|
| 発火の規則 | 380 | 866 |
| **実際に組み立てているイベントの型** | **133** | 248 |
| `patches/events/generated` の規則ファイル | 91 | 197(うち 118 は同じパスのファイルが 1.20.6 にもある) |

残り 119 の一覧は [backlog/events-missing.txt](backlog/events-missing.txt)。
うち 28 は 1.21 以降の API とみられるので、**実際に写すのは 91 件**。
写す手順もそこに書いてある。
