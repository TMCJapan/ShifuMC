...
| ブロックの破壊と設置のイベントが 1 つも出ない | 発火層のメソッドごと落ちていた(`BlockBreakEvent` は import だけが残っていた)。保護系のプラグインはこれで一切効かない | `main` から写して 1.20.6 の行へ。設置は vanilla のとおり先に置いてから発火し、取り消されたときだけ控えへ戻す |
| 移動のイベントが 1 つも出ない | 同じく発火層ごと。`hand` の `internalTeleport` と `shifuMoveFrom` も無かった | `absMoveTo` の直前で発火。1/256 ブロック・10 度に満たない動きでは出さない(Paper と同じ) |
| 湧きのイベントが 1 つも出ない | `patches/events/entity-spawn.rules` が空だった。`CreatureSpawnEvent` / `ItemSpawnEvent` / `ProjectileLaunchEvent` / `VehicleCreateEvent` / `LightningStrikeEvent` / `EntitySpawnEvent` は全部ここから出る | vanilla の `addEntity(Entity)` が `entityManager` へ入れる直前で `doEntityAddEventCalling` を呼ぶ。**登録が無ければ呼ばない**(組み立てが分類に関わらず走るため)。bot で 65 件 / 102 件 |
| kick した瞬間に NPE | 1.20.6 の `ServerGamePacketListenerImpl` は `player` を自分でも宣言していて、親(`ServerCommonPacketListenerImpl`)の同名の欄を隠す。親の方を読む 4 箇所が全部 null | 構築子とリスポーンの 2 箇所で `super.player` にも入れる |
...
`make_events.py` が読めず、機械で出せるのは 21 件しかない。Bukkit のイベントは
ほぼ全部その 543 件の側にある。そこで `main`(26.2)の規則を移した。

| | 件数 |
|---|---|
| アンカーがそのまま当たる | 31 |
| `reanchor.py` が局所変数の名前だけ当て直して通る | 66 |
| 近い行が無い | 81 |
| ファイルごと無い | 52 |

写した 97 件のうち `settle_events.sh` を通ったのは 34 件。そのあと、落ちた分の
「アンカーは当たるが本体が 26.2 の局所変数を呼ぶ」を 1 件ずつ直して 10 件戻した
(`reanchor.py` はアンカーが当たる規則を見ないので、本体だけ古い名前が残る)。

残る 121 の型は、発火層のメソッドごと落ちているものが多い。`main` の発火層は
メソッド 400、1.20.6 は 215。**空のクラスだけが残っているものもある**
(`InventoryClicks` は中身が無く、`BlockBreakEvent` は import だけがあった)。
1 件ずつ Paper の 1.20.6 のハンクを読んで戻すことになる。
...
同じ数字。残る 1 の位置は走るたびに変わる。

発火 577 箇所(2026-09-10)では、vanilla を 4 回走らせて変わる 108 チャンクを
両側から引いたうえで **比べたチャンク 736 / 一致 736、不一致 0**。
...
発火 577 箇所(2026-09-10)では、vanilla 同士のばらつき 112 チャンクを引いて
**比べたチャンク 732 / 一致 732、不一致 0**。Level.setBlock の先頭に
木の生成の控えを足したあとも変わらない(控えは聞き手がいるときだけ立つ)。
