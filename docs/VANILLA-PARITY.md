...
| `patches/decompile/exprs.rules` — 逆コンパイラが意味を変えた式を戻す | 4 件 | 公式と同じ命令列に戻す。`postcompile.sh` の SemDiff が名指ししたメソッドを公式と比べ、違えば組むのを止める |
...
| 比べたもの | ばらつき(vanilla 4 回) | ばらつき(Shifu 2 回) | 比べたチャンク | 違うチャンク |
|---|---|---|---|---|
| 起動して 20 秒 | 110 | — | 733 | **0** |
| 1200 tick(種と tick 数を固定) | 108 | 3 | 733 | **0** |

**両方が再現できるチャンクでは、Shifu と vanilla の違いが 1 つも無い。**
(2026-09-09 に測り直し。発火 357 / 配線 67 を入れた状態)

vanilla を 2 回でなく 4 回走らせるのは、2 回だと「たまたま揃ったチャンク」が
ばらつきに数えられず、実差のように見えるため。1.20.6 では 2 回のとき 5 件、
3 回で 3 件、4 回で 0 件になった。

**Shifu の側も 2 回走らせて差し引く。** 片側だけ引くと、もう片側のばらつきが
実差に見える。1200 tick では vanilla を 4 回引いても 1 チャンク残り、
Shifu をもう 1 度走らせると**別のチャンク**が残った。両方引いて 0 になる。
...
変わるため。`BlockFertilizeEvent` と `StructureGrowEvent` は、登録があるときだけ
`Level.captureTreeGeneration` を立てて、木の生成で置かれるブロックを押さえて出している
(`patches/events/generated/net-minecraft-world-level-tree-grow.rules`)。登録が無ければ旗は立たない。

### プラグインが無いときの扱いを決めたもの

Bukkit が自分で登録するコマンド。`SimpleCommandMap` は構築子と、`CraftServer.enablePlugins(POSTWORLD)` が呼ぶ
`setFallbackCommands()` で、プラグインの数に関係なく `help`・`reload`・`version`・`plugins`・`timings` を `<名前>` と `bukkit:<名前>` の両方で登録する。
vanilla にもある名前(`/help` と `/reload`)は、プラグインを 1 つも入れていなくても Bukkit 版に置き換わっていた。
...
| `Ingredient.exact` | `Ingredient.test` のループの前 | `CraftRecipe.toIngredient`(`RecipeChoice.ExactChoice`) |
