...
| `patches/decompile/exprs.rules` — 逆コンパイラが意味を変えた式を戻す | 3 件 | 公式と同じ命令列に戻す。`postcompile.sh` の SemDiff が名指ししたメソッドを公式と比べ、違えば組むのを止める |
...
`setFallbackCommands()` で、プラグインの数に関係なく `help`・`reload`・`version`・`timings` を `<名前>` と `bukkit:<名前>` の両方で登録する。
